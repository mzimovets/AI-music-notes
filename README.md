...
## Проверка офлайна перед выкладкой

Приложение обязано работать без связи — в этом весь его смысл, и именно эта
часть ломалась чаще всего. Причём каждый раз поломку находил регент на планшете:
перенести файлы на плату, пересобрать, переустановить приложение, включить режим
самолёта — полчаса на одну проверку. Поэтому правки уезжали непроверенными.

Проверка делает то же самое за полминуты: поднимает настоящий браузер, входит,
ждёт кеширования, отключает связь и проходит по страницам. Ловит ровно те беды,
что случались: подмену страницы главной, служебный текст вместо нот и подвисание
экрана от тяжёлого пересчёта.

```bash
# приложение должно быть запущено (npm run start и server/node index.js)
cd tests
npm install                       # один раз
npx playwright install chromium   # один раз
NOTES_PASSWORD=<пароль регента> npm run check
```

Зависимости у проверки свои и намеренно вынесены из основного `package.json`:
`deploy.sh` ставит и разработческие пакеты, а на плате браузер для проверок не
нужен и весит слишком много.

**Правило: не выкладывать, пока проверка не пройдена.** Особенно перед
выступлением. За несколько дней до концерта правки лучше не вносить вовсе —
любая из них требует проверки, а времени на неё уже не будет.
