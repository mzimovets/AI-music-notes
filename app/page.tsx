  ...
  return (
    <SongsLibraryContextProvider albumsPromise={albumsPromise}>
      ...
      <>
          ...
          <Popover placement="top-start" offset={12} isOpen={isMenuOpen} onOpenChange={setIsMenuOpen}>
            ...
            <PopoverContent classNames={{ base: "p-2 bg-white/40 backdrop-blur-2xl border border-white/50 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.18)]" }}>
              <div className="flex flex-col gap-2">
                ...
                {isRegent && (
                  <button
                    onClick={() => { setIsMenuOpen(false); setIsBatteryModalOpen(true); }}
                    className="w-12 h-12 rounded-full bg-gradient-to-br from-[#BD9673] to-[#7D5E42] flex items-center justify-center shadow-md hover:scale-110 transition-transform"
                ...
                {isRegent && (
                  <button
                    onClick={() => { setIsMenuOpen(false); setIsWifiModalOpen(true); }}
                    className="w-12 h-12 rounded-full bg-gradient-to-br from-[#BD9673] to-[#7D5E42] flex items-center justify-center shadow-md hover:scale-110 transition-transform"
        ...
        {/* Десктопные кнопки — видны только на md+, в строку (landscape) или столбик (portrait) */}
        <div className="hidden md:flex fixed bottom-6 left-6 z-50 flex-row portrait:flex-col gap-3">
          <button
