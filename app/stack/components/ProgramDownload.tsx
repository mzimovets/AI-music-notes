...
const ProgramDownload: React.FC<ProgramDownloadProps> = ({
  backgroundUrl,
  programText,
  width = 2480,
  height = 3508,
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const scalingFactor =
        totalSongCount <= minSongs
          ...
          : totalSongCount >= maxSongs
            ...
            : 0.8 + ((maxSongs - totalSongCount) / (maxSongs - minSongs)) * 0.7; // линейно
      ...
      console.log(`💾 Blob создан: ${(blob.size / 1024 / 1024).toFixed(2)} MB`);

      // Скачиваем файл
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `program-${width}x${height}-${Date.now()}.png`;
  ...
  return (
    <div>
      <button
        className="px-6 py-3 bg-gradient-to-r from-[#BD9673] to-[#7D5E42] text-white rounded-lg font-medium hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed"
        onClick={handleDownload}
        disabled={isLoading}
      >
        {isLoading
          ? `Создание изображения ${width}×${height}...`
          : `Скачать программу (PNG ${width}×${height})`}
      </button>
    </div>
  );
};
