...
import { useShareSong } from "./hooks/ShareSong";
import { useDownloadSong } from "./hooks/DownloadSong";
import { usePrintSong } from "./hooks/PrintSong";
...
export const SearchTableCell = ({
  ...
  const { handleDownload } = useDownloadSong();
  const { handlePrint, isLoading: PrintElement } = usePrintSong();

  switch (columnKey) {
    case "name":
      return (
        <Link className="text-foreground" href={`/song/${song._id}`}>
