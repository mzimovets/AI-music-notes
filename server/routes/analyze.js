...
async function analyzeWithClaude(songName, pdfText, webLyrics = null) {
  ...
  // Веб — основной источник текста (чистый, без SATB-артефактов)
  const webSection = webLyrics
    ? `\n\nТекст песни из интернета:\n---\n${webLyrics.slice(0, 1500)}\n---`
    : "";

  // PDF — последний резерв
  const pdfSection = hasRealText && !webLyrics
    ? `\n\nТекст из PDF нот (может содержать ошибки SATB-нотации):\n---\n${pdfText.slice(0, 1000)}\n---`
    : "";

  const lyricsInstruction = webLyrics
    ? `Поле lyrics — приоритеты:
1. Если текст из интернета выше точно соответствует песне «${songName}» — скопируй его.
2. Если текст явно не подходит (другая песня, проза, навигация) — напиши текст из своих знаний.
3. Если не знаешь текст — оставь поле пустым "".`
    : hasRealText
      ? `Поле lyrics — приоритеты:
1. Если знаешь текст песни «${songName}» из своих обучающих данных — напиши его.
2. Если не знаешь — восстанови из PDF выше (исправь ошибки SATB-нотации).
3. Если не уверен — оставь поле пустым "".`
      : `Поле lyrics: если знаешь текст песни «${songName}» — напиши его. Если не уверен — оставь пустым "".`;
