...
async function analyzeWithClaude(songName, pdfText, webLyrics = null) {
  ...
  const lyricsInstruction = webLyrics
    ? `Поле lyrics: если текст из интернета выше точно соответствует песне «${songName}» — скопируй его. Если текст явно не подходит (другая песня, проза, навигация) — оставь поле пустым "". НЕ придумывай текст из своих знаний.`
    : hasRealText
      ? `Поле lyrics: восстанови текст из PDF выше (исправь ошибки SATB-нотации). Если текст нечитаемый — оставь пустым "". НЕ придумывай текст из своих знаний.`
      : `Поле lyrics: оставь пустым "". НЕ придумывай текст.`;
