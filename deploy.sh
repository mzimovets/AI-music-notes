...
if [ "$FETCH" = "1" ]; then
  ...
  # Не git pull, а полное приведение к состоянию репозитория.
  #
  # pull старается сохранить местные изменения и упирается в них: то файлы
  # блокировок переписал npm install, то файлы принесли на плату вручную через
  # tar. Каждый раз обновление вставало и требовало разбирательства, хотя
  # сохранять там нечего — ни сервер, ни плата не место для правки кода.
  #
  # reset --hard затирает и изменённые, и принесённые файлы. Данные при этом в
  # безопасности: база, ноты и настройки репозиторием не ведутся.
